...
public class CustomSeekBar extends View {
    ...
    private Rect mSectionTvRect;
    private Drawable mThumbDrawable = null;

    private String[] mSectionTv = {"0.5x", "0.75x", "1.0x", "1.5x", "2.0x"};
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.EXACTLY && heightSize > 120) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.translate(mViewWidth / 2, (mViewHeight / 4) * 3);
    ...
    private void moveToPoint(float coord) {
        float halfLength = mProgressLength / 2 - 50;
        if (coord > halfLength) {
            coord = halfLength;
            mIsMovedOut = true;
    ...
    private float progress2Coord(int progress) {
        if (HORIZONTAL == mOrientation) {
            return (float) (mProgressLength - 100) * (progress - mMinProgress)
                    / (mMaxProgress - mMinProgress) - (mProgressLength - 100) / 2f;
        } else {
            return (mProgressLength - 100) / 2 - (float) (mProgressLength - 100) * (progress - mMinProgress)
    ...
    private int coord2Progress(int coord) {
        if (coord > (mProgressLength - 100) / 2) {
            ...
            if (HORIZONTAL == mOrientation) {
                return Math.round((coord + (mProgressLength - 100) / 2f)
